...
static unsigned parse_field_selection(const char *text,
...
static int compare_idf_entry(const void *left, const void *right) {
  const IdfEntry *a = left;
  const IdfEntry *b = right;
  return strcmp(a->pattern, b->pattern);
}

static void idf_table_free(IdfTable *table) {
...
static void invalid_idf(const char *path, size_t line_number,
                        const char *detail) {
  if (line_number > 0)
    fprintf(stderr, "%s: %s:%zu: %s\n", PROG_NAME, path, line_number, detail);
  else
    fprintf(stderr, "%s: %s: %s\n", PROG_NAME, path, detail);
  fprintf(stderr, "%s: IDF file is invalid; regenerate it with cw --idf-out\n",
          PROG_NAME);
  exit(EXIT_FAILURE);
}

static void idf_table_append(IdfTable *table, const char *pattern, double idf) {
  if (table->len == table->cap) {
    size_t new_cap = table->cap == 0 ? 256 : table->cap * 2;
    table->items = xrealloc(table->items, new_cap * sizeof(*table->items));
    table->cap = new_cap;
  }
  table->items[table->len].pattern = xstrdup(pattern);
  table->items[table->len].idf = idf;
  table->len++;
}

static IdfTable read_idf_file(const char *path) {
  ...
  while (getline(&line, &capacity, stream) != -1) {
    line_number++;
    char *saveptr = NULL;
    char *pattern = strtok_r(line, " \t\r\n", &saveptr);
    char *value_text = strtok_r(NULL, " \t\r\n", &saveptr);
    char *extra = strtok_r(NULL, " \t\r\n", &saveptr);

    if (pattern == NULL || value_text == NULL || extra != NULL)
      invalid_idf(path, line_number, "expected exactly: pattern idf");

    char *end = NULL;
    errno = 0;
    double value = strtod(value_text, &end);
    if (errno != 0 || end == value_text || *end != '\0' || !isfinite(value) ||
        value < 0.0)
      invalid_idf(path, line_number, "invalid IDF value");

    idf_table_append(&table, pattern, value);
  }

  if (ferror(stream)) {
    free(line);
    idf_table_free(&table);
    fclose(stream);
    die_errno(path);
  }
  free(line);
  ...
  if (table.len == 0)
    invalid_idf(path, 0, "empty IDF file");

  qsort(table.items, table.len, sizeof(*table.items), compare_idf_entry);
  for (size_t i = 1; i < table.len; i++) {
    if (strcmp(table.items[i - 1].pattern, table.items[i].pattern) == 0)
      invalid_idf(path, 0, "duplicate pattern");
  }
  return table;
}

static const IdfEntry *find_idf(const IdfTable *table, const char *pattern) {
  IdfEntry key = {.pattern = (char *)pattern, .idf = 0.0};
  return bsearch(&key, table->items, table->len, sizeof(*table->items),
                 compare_idf_entry);
}

static void apply_external_idf(CwtCorpusStats *stats, const IdfTable *table,
                               const char *path) {
  for (size_t i = 0; i < stats->token_count; i++) {
    CwtTokenStat *token = &stats->tokens[i];
    const IdfEntry *entry = find_idf(table, token->pattern);
    if (entry == NULL) {
      fprintf(stderr, "%s: %s: missing IDF for pattern '%s'\n", PROG_NAME, path,
              token->pattern);
      fprintf(stderr,
  ...
  for (size_t i = 0; i < stats->pair_count; i++) {
    ...
    const IdfEntry *left = find_idf(table, pair->pattern1);
    const IdfEntry *right = find_idf(table, pair->pattern2);
    if (left == NULL || right == NULL) {
      fprintf(stderr, "%s: %s: missing IDF for pair pattern\n", PROG_NAME,
              path);
      fprintf(stderr, "%s: regenerate the IDF file with cw --idf-out\n",
              PROG_NAME);
      exit(EXIT_FAILURE);
    }
    pair->idf1 = left->idf;
...
static void print_idf(const CwtCorpusStats *stats) {
  for (size_t i = 0; i < stats->token_count; i++)
    printf("%s\t%.17g\n", stats->tokens[i].pattern, stats->tokens[i].idf);
}
...
static SelectedFrequencyIndex
build_selected_frequency_index(const CwtCorpusStats *stats,
                               const StringSetBuilder *key_units) {
  SelectedFrequencyIndex index = {0};
  if (stats->token_count == 0)
    return index;

  index.items = xrealloc(NULL, stats->token_count * sizeof(*index.items));
...
static const SelectedTokenFrequency *
find_selected_frequency(const SelectedFrequencyIndex *index,
                        const char *pattern) {
  SelectedTokenFrequency key = {.pattern = pattern};
  return bsearch(&key, index->items, index->len, sizeof(*index->items),
                 compare_selected_frequency);
}
...
static void print_pairs(const CwtCorpusStats *stats,
  ...
  for (size_t i = 0; i < stats->pair_count; i++) {
    ...
    printf("%s\t%s\t%zu\t%zu\t%zu\t%.12g\t", pair->token1, pair->token2, ctf,
           cdf, pair->df1, pair->idf1);
...
static void process_pairs(const CwtCorpusStats *stats,
  ...
  print_pairs(stats, key_units, &distribution, &frequencies, method, key_fq);
...
int main(int argc, char **argv) {
  ...
  unsigned pattern_fields =
      CWT_TOKEN_FIELD_BIT(2) | CWT_TOKEN_FIELD_BIT(3) | CWT_TOKEN_FIELD_BIT(4);
  regex_t key_regex;
  ...
  while ((option = getopt_long(argc, argv, "p:k:M:shv", long_options, NULL)) !=
    ...
    case 'M': {
      ...
      if (errno != 0 || end == optarg || *end != '\0' ||
          (value != CW_METHOD_BASIC && value != CW_METHOD_WAKA_GRAPH &&
           value != CW_METHOD_RARE_PATTERN && value != CW_METHOD_GLOBAL_PAIR)) {
        fprintf(stderr, "%s: --method requires 1, 7, 12, or 16 (got '%s')\n",
                PROG_NAME, optarg);
        return EXIT_FAILURE;
      }
    ...
    case OPT_IDF_IN:
      ...
      break;
    case 's':
      pattern_fields = CWT_TOKEN_FIELD_BIT(1) | CWT_TOKEN_FIELD_BIT(2) |
  ...
  const CwtStatsReadOptions read_options = {.pattern_fields = pattern_fields,
                                            .pattern_substr = pattern_substr,
                                            .unordered_pairs = 1,
                                            .unique_pairs_per_unit = 1,
                                            .drop_self_pairs = 1};

  cwt_stats_read_with_options(stream, &stats, PROG_NAME, &read_options);

  IdfTable idf_table = {0};
  if (idf_in_path != NULL) {
    idf_table = read_idf_file(idf_in_path);
    apply_external_idf(&stats, &idf_table, idf_in_path);
  }
