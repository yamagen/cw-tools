                        ...
                        const char *detail)
{
    ...
    fprintf(stderr,
            "%s: IDF file is invalid; regenerate it with "
            "cw --idf-out\n", PROG_NAME);
    exit(EXIT_FAILURE);
}
...
static void idf_table_append(IdfTable *table, const char *pattern, double idf)
{
    if (table->len == table->cap) {
        ...
        table->items = xrealloc(table->items,
                                new_cap * sizeof(*table->items));
        table->cap = new_cap;
    }
...
static IdfTable read_idf_file(const char *path)
{
    ...
    while (getline(&line, &capacity, stream) != -1) {
        ...
        if (pattern == NULL || value_text == NULL || extra != NULL)
            invalid_idf(path, line_number,
                        "expected exactly: pattern idf");
...
static const IdfEntry *find_idf(const IdfTable *table, const char *pattern)
{
    ...
    return bsearch(&key, table->items, table->len,
                   sizeof(*table->items), compare_idf_entry);
}
...
static SelectedFrequencyIndex build_selected_frequency_index(
    const CwtCorpusStats *stats, const StringSetBuilder *key_units)
{
    ...
    for (size_t i = 0; i < stats->token_count; i++) {
        ...
        index.items[i].fq = stats->tokens[i].fq_available
            ? selected_token_frequency(&stats->tokens[i], key_units) : 0;
    }
...
typedef struct { size_t count; double mean; double sd; } CwDistribution;
                        ...
                        CwMethod method, size_t key_fq)
{
    for (size_t i = 0; i < stats->pair_count; i++) {
        ...
        printf("%s\t%s\t%zu\t%zu\t%zu\t%.12g\t",
               pair->token1, pair->token2, ctf, cdf,
               pair->df1, pair->idf1);
...
static void print_help(FILE *stream)
{
    fprintf(stream,
            ...
            "  -k, --key REGEX  restrict occurrences using a POSIX ERE\n"
            "  -M, --method N   CW method: 1, 7, 12, or 16 (default: 7)\n"
            "      --idf-out    write pattern and locally calculated IDF to stdout\n"
            "                   and exit\n"
            "      --idf-in FILE\n"
            "                   use IDF values previously written by --idf-out\n"
...
{
    enum { OPT_IDF_OUT = 256, OPT_IDF_IN };
    ...
    while ((option = getopt_long(argc, argv, "p:k:M:shv",
                                 long_options, NULL)) != -1) {
        switch (option) {
        case 'p':
