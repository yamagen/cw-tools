...
static struct node *table_get(struct table *table, const char *key,
  ...
  for (struct node *node = table->bucket[h]; node != NULL; node = node->next) {
...
static void frequency_add(struct node *node, const char *unit, size_t count) {
  ...
  if (frequencies->len > 0 &&
    ...
    if (frequencies->len == frequencies->cap) {
      ...
      frequencies->unit =
          xrealloc(frequencies->unit, new_cap * sizeof(*frequencies->unit));
      frequencies->count =
          xrealloc(frequencies->count, new_cap * sizeof(*frequencies->count));
...
static void process_line(char *line, size_t line_number, const char *source,
  ...
  if (left_input == NULL || right_input == NULL || extra != NULL ||
    ...
    fprintf(stderr, "%s: %s:%zu: expected: unit_id token1 token2 [fq1 fq2]\n",
  ...
  if (options != NULL) {
    ...
    pattern_substr = options->pattern_substr;
  ...
  if (pattern_substr > 0) {
    if (strlen(left_key) > pattern_substr)
      left_key[pattern_substr] = '\0';
    if (strlen(right_key) > pattern_substr)
      right_key[pattern_substr] = '\0';
  }
  ...
  if (record_variant(left_term, left_input, unit_key))
    record_term_frequency(left_term, unit_key, left_count, counts_available);
  if (record_variant(right_term, right_input, unit_key))
    record_term_frequency(right_term, unit_key, right_count, counts_available);
...
static void copy_tokens(CwtCorpusStats *stats, struct table *terms,
  ...
  for (size_t i = 0; i < terms->size; i++) {
    for (struct node *node = terms->bucket[i]; node != NULL;
         node = node->next) {
      ...
      if (token->fq_available) {
        token->fq = node->fq;
        token->fq_unit_ids.items =
            xcalloc(node->frequencies.len, sizeof(*token->fq_unit_ids.items));
        token->fq_unit_counts =
            xcalloc(node->frequencies.len, sizeof(*token->fq_unit_counts));
        ...
        for (size_t j = 0; j < node->frequencies.len; j++) {
          token->fq_unit_ids.items[j] = xstrdup(node->frequencies.unit[j]);
...
static void copy_pairs(CwtCorpusStats *stats, struct table *pairs,
  ...
  for (size_t i = 0; i < pairs->size; i++) {
    for (struct node *node = pairs->bucket[i]; node != NULL;
         node = node->next) {
      array[k++] = node;
    }
  }
...
void cwt_stats_init(CwtCorpusStats *stats) { memset(stats, 0, sizeof(*stats)); }
